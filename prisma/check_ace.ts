// Use npx tsx to run this
process.chdir('G:\\CodeBase\\cpro-website');
...
(async () => {
  ...
  for (const p of ps) {
    console.log(`${p.name.padEnd(28)} | id=${p.id.padEnd(40)} | isFeatured=${p.isFeatured} | isActive=${p.isActive} | tier=${(p.tier||'').padEnd(20)} | score=${p.score} | dl=${p.downloadCount}`);
  }
