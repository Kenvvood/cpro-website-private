"use client";
// PipValueCalculator — 点值 & 盈亏 (XAUUSD 实战)
// 输入: 开仓价 / 平仓价 / 仓位手数 / 品种
// 输出: 每点价值 + 实际盈亏 (多空方向)
import { useState } from "react";

type Direction = "long" | "short";
type Instrument = "XAUUSD" | "EURUSD" | "GBPUSD" | "USDJPY";
...
const INSTRUMENTS: Record<Instrument, { contractSize: number; pipMultiplier: number; name: string }> = {
  ...
  EURUSD: { contractSize: 100000, pipMultiplier: 0.0001, name: "欧元 (1 手 = 100,000)" },
  GBPUSD: { contractSize: 100000, pipMultiplier: 0.0001, name: "英镑 (1 手 = 100,000)" },
  USDJPY: { contractSize: 100000, pipMultiplier: 0.01,  name: "日元 (1 手 = 100,000)" },
};
...
export function PipValueCalculator() {
  ...
  // 移动点数 (XAUUSD 1 pip = 0.1, EURUSD 1 pip = 0.0001)
      ...
      {valid && (
        <div className="card-base overflow-hidden">
          ...
          <div className="px-5 py-3 border-t border-border bg-bg-tertiary text-[10px] text-text-muted leading-relaxed">
            <strong>简化计算:</strong> XAUUSD/EURUSD/GBPUSD/USDJPY 每手 1 pip 标准价值约 $10。
