"use client";
// AuthorInsights — 作者分享 (v22.0 Phase 7.10 重构)
// PM 决策: E (作者分享) + F (工作流) 上下合并到左侧, 右侧 AuthorSidebar 独立 (高度对齐左侧)
// 之前: 自己带 1fr_280px grid, 双 grid 嵌套
// 现在: 只输出主表 (h2 + 3 tab + 10 行文章), 侧栏交给 page.tsx 调 AuthorSidebar
...
const TABS: { key: Category; label: string; desc: string }[] = [
  ...
  { key: "market", label: "市场观察", desc: "XAUUSD 黄金 / 黄金套利对 行情心得" },
...
const POSTS: Post[] = [
  ...
  { date: "2026-07-15", category: "market", title: "XAUUSD/JPY 套利对 日本央行决议前后的 4 小时窗口规律",
    excerpt: "近 6 次日本央行决议 XAUUSD/JPY 在 ±2% 区间的反应, 总结 3 类入场机会的胜率。", readMin: 7, views: 1024 },
...
export function AuthorInsights() {
  ...
  return (
    <section>
      {/* 区块头: h2 + 副标 + 本周新增徽章 + 查看全部 (跟 HowItWorks 对称) */}
      <div className="flex justify-between items-end mb-3 flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-text-muted">
            站长 山治廿一 · 量化交易架构师 · CProTrading 城诺科技 · 最近更新 2026-08-08
          </p>
        </div>
      ...
      {/* 主表 (3 tab 切换 + 10 行文章) - 去掉自己 1fr_280px grid, 单列占满 page.tsx 给的 1fr */}
      ...
      <div className="text-[10px] text-text-muted leading-relaxed border-t border-border pt-3">
        所有文章均为 山治廿一 真实开发与交易经验。 引用请注明 CProTrading 城诺科技。 不构成投资建议。
      </div>
    </section>
  );
}
